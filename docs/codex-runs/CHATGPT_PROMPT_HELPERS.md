...
6. After Codex writes a result, run `codex-run-ledger review --slice-id <slice_id> --write-review-summary --markdown` and ask ChatGPT to review with `REVIEW_PROTOCOL.md` using the prompt, result, review packet, final diff, commits, and verification evidence.
